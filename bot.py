...
class Memory_Entity():
    ...
    def entity_generate(self,history: Dict[str, Any],summary_history) -> Dict[str, Any]:
        ...
        with get_openai_callback() as cb:
            ...
            total_coin += coin
            logger.info(Fore.YELLOW + f"coast {cb.total_cost}, cost coins {coin}, spend prompt: {cb.total_tokens}")
    ...
    def entity_summary(self,history: Dict[str, Any],summary_history) -> Dict[str, Any]:
        ...
        for entity in self.entity_cache:
            ...
            with get_openai_callback() as cb:
                ...
                coin = (math.floor((math.floor(cb.prompt_tokens * 3 / 8) + math.floor(cb.completion_tokens / 2))) * 0.4 ) + 1
                total_coin += coin
                logger.info(Fore.YELLOW + f"coast {cb.total_cost}, cost coins {coin}, spend prompt: {cb.total_tokens}"+ Fore.RESET)
...
def send_chatgpt_request(send_msg):
    ...
    try:
        with get_openai_callback() as cb:
            ...
            logger.info(Fore.YELLOW + f'coast {round(total_coast,3)}￥, coast coins {int(coin)} - total {int(total_coin)}, Spend prompt: {cb.prompt_tokens} tokens | completion: {cb.completion_tokens} tokens' + Fore.RESET)
