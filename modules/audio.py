...
def audio_to_text(path: str):
    ...
    if audio_len > 0.1:
        ...
        end_time = time.time()
        main.logger.info(Fore.GREEN + 'Result: "{}".'.format(result) + Style.RESET_ALL)
        ...
        if result != "":
            ...
            main.logger.warning(Back.RED + "输入为空" + Style.RESET_ALL)
            return None
    else:
        main.logger.warning(Back.RED + "语音时间不足0.1s" + Style.RESET_ALL)
        return None
...
def listen(audio_path: str):
    ...
    while not leave:
        ...
        record_to_file(audio_path, raw_data, 2)
        leave = True
